...
@SpringBootTest
public class InvoicingServiceLayerTest {
    private CatalogClient client;
    private InvoiceRepository invoiceRepository;
    private TaxRepository taxRepository;
    private ProcessingFeeRepository processingFeeRepository;
    private InvoicingServiceLayer service;
    ...
    public void setUp() throws Exception {
        ...
        service = new InvoicingServiceLayer(client, invoiceRepository, taxRepository, processingFeeRepository);
    }
    ...
    public void shouldCreateGetConsole() {
        ...
        ConsoleViewModel console1 = service.getConsole(console.getId());
...
//    //Testing Game operations...
    ...
    private void setUpClientMock(){
        ...
        doReturn(savedConsole1).when(client).saveConsole(newConsole1);
        doReturn(savedConsole2).when(client).saveConsole(newConsole2);
        doReturn(savedConsole3).when(client).saveConsole(newConsole3);
        doReturn(savedConsole1).when(client).getConsoleById(40L);
        doReturn(consoleByManufacturer).when(client).selectConsoleByManufacturer("Sony");
        ...
        doReturn(savedGame1).when(client).saveGame(newGame1);
        doReturn(savedGame3).when(client).getGameById(60L);
        doReturn(savedGame1).when(client).getGameById(32L);
        doReturn(savedGame2).when(client).getGameById(25L);
        doReturn(savedGame2).when(client).saveGame(newGame2);
        doReturn(savedGame3).when(client).saveGame(newGame3);

        doReturn(gamesByEsrb).when(client).selectGameByEsrbRating("E10+");
        doReturn(gamesByStudio).when(client).selectGameByStudio("Dolby Studios");
        doReturn(gamesByTitle).when(client).selectGameByTitle("Halo");
        ...
        doReturn(savedTShirt1).when(client).saveTShirt(newTShirt1);
        doReturn(savedTShirt2).when(client).saveTShirt(newTShirt2);
        doReturn(savedTShirt3).when(client).saveTShirt(newTShirt3);
        doReturn(savedTShirt3).when(client).getTShirtById(72L);
        doReturn(savedTShirt1).when(client).getTShirtById(54L);
        doReturn(savedTShirt4).when(client).getTShirtById(99L);

        doReturn(tShirtsByColor).when(client).selectTShirtByColour("Blue");
        doReturn(tShirtsBySize).when(client).selectTShirtBySize("Medium");
    ...
    private void setUpProcessingFeeRepositoryMock() {
        ...
        doReturn(processingFee).when(processingFeeRepository).findByProductType("T-Shirt");
    ...
    private void setUpTaxRepositoryMock() {
        ...
        doReturn(taxNC).when(taxRepository).findByState("NC");
        doReturn(taxNY).when(taxRepository).findByState("NY");
